...
# The following 2 lines were commented out by Pablo. mpl version differences?
...
def process_array(array, network, T0):
    ...
    if "OUT_VALVE_DIR" in dir(config):
        try:
            ...
            t = np.array(
                [
                    UTCDateTime(dates.num2date(ti)).strftime("%Y-%m-%d %H:%M:%S")
                    for ti in t
                ]
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:  # No time given. Run from cron. Use current time
        ...
        len(sys.argv) == 2
    ):  # time given as single string YYYYMMDDhhmm (eg. 201705130301)
        T0 = sys.argv[1]
        try:
            T0 = UTCDateTime(T0)
            T0 = UTCDateTime(
                T0.strftime("%Y-%m-%d %H:%M")[:-1] + "0"
            )  # round down to the nearest 10-minute
        except:
            warnings.warn(
    ...
    print(
        "Processing {} - {}".format(
            (T0 - config.DURATION).strftime("%Y.%m.%d %H:%M"),
    ...
    for network in config.NETWORKS:
        ...
        for array in network["ARRAYS"]:
            ...
            print(
                "{:.1f} seconds to process {}".format(
                    time.time() - timer_tmp, array["Name"]
                )
            )
