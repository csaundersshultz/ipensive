...
def write_to_log(day):
    ...
    file = (
        ...
        + month
        + "/"
        + UTCDateTime(day).strftime("%Y-%m-%d")
        + ".log"
    )
...
def add_coordinate_info(st, SCNL):
    ...
    for tr in st:
        if tr.stats.location == "":
            tr.stats.location = "--"
        tmp_scnl = "{}.{}.{}.{}".format(
            tr.stats.station, tr.stats.channel, tr.stats.network, tr.stats.location
        )
        tmp_lat = SCNL[SCNL["scnl"] == tmp_scnl].sta_lat.values[0]
        tmp_lon = SCNL[SCNL["scnl"] == tmp_scnl].sta_lon.values[0]
        tr.stats.coordinates = AttribDict(
            {"latitude": tmp_lat, "longitude": tmp_lon, "elevation": 0.0}
        )
    return st
...
def setup_coordinate_system(st):
    ...
    yx = (
        R
        * np.array(
            [lats * np.pi / 180.0 - lat0, (lons * np.pi / 180.0 - lon0) * np.cos(lat0)]
        ).T
    )
...
def inversion(st):
    ...
    dt = lags
    Dm3 = np.array(
        [ds * np.cos(az * (np.pi / 180.0)), ds * np.sin(az * (np.pi / 180.0))]
    ).T
    ...
    sv_new = np.array(
        [
            np.cos(azimuth * np.pi / 180) / velocity,
            np.sin(azimuth * np.pi / 180) / velocity,
        ]
    )
...
### ADD FUNCTION grab_data_local(), make sure output is formatted the same
def grab_data_local(filename):
    st = read(filename)
    return st


def grab_data(scnl, T1, T2, hostname, port, fill_value=0, grab_local=False):
    ...
    for sta in scnl:
        ...
        try:
            ...
            print("Blank trace! data dl failed")
            tr = Stream()
...
def write_ascii_file(t2, t, pressure, azimuth, velocity, mccm, rms, name):
    ...
    if os.path.exists(filename):
        ...
        df = df[
            (df["Time"] <= t1.strftime("%Y-%m-%d %H:%M:%S"))
            | (df["Time"] > t2.strftime("%Y-%m-%d %H:%M:%S"))
        ]
        df = pd.concat([df, tmp])
...
def write_valve_file(t2, t, pressure, azimuth, velocity, mccm, rms, name):
    ...
    filename = (
        config.out_valve_dir + "/" + name + "_" + t2.strftime("%Y%m%d-%H%M") + ".txt"
    )
...
def plot_results(t1, t2, t, st, mccm, velocity, azimuth, array, network):
    ...
    params_tmp = {
        ...
        if var not in ["NETWORKS"] and "__" not in var
    }
    ...
    axs1b.set_yticks([])
    axs1b.set_ylabel(
        "{:.1f} - {:.1f} Hz".format(params_tmp["FREQMIN"], params_tmp["FREQMAX"]),
        labelpad=6,
    )

    axs2 = plt.subplot(4, 1, 2)
    sc = plt.scatter(t, mccm, c=mccm, edgecolors="k", lw=0.3, cmap=cm)
    axs2.plot(
        [T1, T2], [params_tmp["MCTHRESH"], params_tmp["MCTHRESH"]], "--", color="gray"
    )
    axs2.axis("tight")
    axs2.set_xlim(T1, T2)
    axs2.set_ylim(cax)
    ...
    rect = Rectangle(
        (T1, params_tmp["VEL_MIN"]),
        T2 - T1,
        params_tmp["VEL_MAX"] - params_tmp["VEL_MIN"],
    )
    pc = PatchCollection([rect], facecolor="gray", alpha=0.25, edgecolor=None)
    plt.gca().add_collection(pc)
    sc = axs3.scatter(t, velocity, c=mccm, edgecolors="k", lw=0.3, cmap=cm)
    if params_tmp["ARRAY_LABEL"] == "Hydroacoustic":
        axs3.set_ylim(1.2, 1.8)
    else:
        axs3.set_ylim(0.15, 0.6)
    axs3.set_xlim(T1, T2)
    sc.set_clim(cax)
    axs3.xaxis_date()
    ...
    if params_tmp["AZ_MAX"] < params_tmp["AZ_MIN"]:
        ...
        for volc in array["volcano"]:
            if volc["back_azimuth"] > 180:
                axs4.plot(
                    [T1, T2],
                    [volc["back_azimuth"] - 360, volc["back_azimuth"] - 360],
                    "--",
                    color="gray",
                    zorder=-1,
                )
                ...
                axs4.plot(
                    [T1, T2],
                    [volc["back_azimuth"], volc["back_azimuth"]],
                    "--",
                    color="gray",
                    zorder=-1,
                )
    ...
    sc = axs4.scatter(t, azimuth, c=mccm, edgecolors="k", lw=0.3, cmap=cm, zorder=1000)
    ...
    cbaxes = fig1.add_axes([0.91, cbot, 0.02, ctop - cbot])
    ...
    sc = plt.scatter(
        t, mccm, s=8 * np.ones_like(t), c=mccm, edgecolors="k", lw=0.1, cmap=cm
    )
    ax_2.plot(
        [T1, T2],
        [params_tmp["MCTHRESH"], params_tmp["MCTHRESH"]],
        "--",
        color="gray",
        linewidth=1,
    )
    ...
    plt.gca().add_collection(pc)
    sc = ax_3.scatter(
        t, velocity, s=8 * np.ones_like(t), c=mccm, edgecolors="k", lw=0.1, cmap=cm
    )
    if params_tmp["ARRAY_LABEL"] == "Hydroacoustic":
        ax_3.set_ylim(1.2, 1.8)
    else:
        ax_3.set_ylim(0.15, 0.6)
    ax_3.set_xlim(T1, T2)
    sc.set_clim(cax)
    ax_3.set_xticks([])
    ax_3.set_yticks([])
    ...
    sc = ax_4.scatter(
        t, azimuth, s=8 * np.ones_like(t), c=mccm, edgecolors="k", lw=0.3, cmap=cm
    )
    ...
    filename = (
        d2 + "/" + array["Name"] + "_" + t2.strftime("%Y%m%d-%H%M") + "_thumb.png"
    )
